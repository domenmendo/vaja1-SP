...
class users_controller
{
    ...
    function update(){
        ...
        //Podatki so pravilno izpolnjeni, registriraj uporabnika
        else if($user->update($_POST["username"], $_POST["email"], $_POST["password"])){
            header("Location: /");
        }
        //Prišlo je do napake pri registraciji
        else{
    ...
    function changePassword(){
        ...
        if(password_verify($_POST["current_password"], $user->password)){
            // If the current password is correct, allow the user to change it
            if($user->changePassword($_POST["new_password"])){
                header("Location: /index.php");
            }
            else{
                header("Location: /users/change_pwd?error=1"); 
            }
        } else {
            // If the current password is incorrect, redirect to the change password page with an error
            header("Location: /users/change_pwd?error=2"); 
        }
        die();
    }
    ...
    function change_pwd() {
        ...
        require_once('views/users/change_pwd.php');
    }
}