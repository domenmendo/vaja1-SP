...
class Article
{
    ...
    public static function create($title, $abstract, $text, $date, $user_id)
    {
        $db = Db::getInstance();
        $stmt = $db->prepare("INSERT INTO articles (title, abstract, text, date, user_id) VALUES (?, ?, ?, ?, ?)");
        $stmt->bind_param("ssssi", $title, $abstract, $text, $date, $user_id);
        $stmt->execute();
        $stmt->close();
    }
    ...
    {
        $db = Db::getInstance();
        $stmt = $db->prepare("UPDATE articles SET title = ?, abstract = ?, text = ? WHERE id = ?");
        $stmt->bind_param("sssi", $title, $abstract, $text, $id);
        $stmt->execute();
        $stmt->close();
    }
